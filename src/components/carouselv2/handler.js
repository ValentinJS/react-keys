...
export function horizontalNestedScrollHandler(id, nestedTranslateX) {
  ...
  _updateBinder(id, { nestedScrollableRefs });
  nestedScrollableRefs[iFocused].ref.style.transform = `translate3d(${nestedTranslateX}px, 0, 0)`;
}
...
export function addScrollableItemRef(id, itemIndex, scrollableItemRef) {
  ...
  if (direction === CAROUSEL_DIRECTIONS.vertical || direction === CAROUSEL_DIRECTIONS.verticalBidirectional) {
...
export function addNestedScrollableItemRef(id, itemIndex, parentItemIndex, scrollableItemRef) {
  ...
  nestedScrollableItems[parentItemIndex][itemIndex].offsetWidth = scrollableItemRef.offsetWidth;
  nestedScrollableItems[parentItemIndex][itemIndex].offsetLeft = scrollableItemRef.offsetLeft;
...
export function getNestedIFocused(id, iFocused) {
  const { nestedScrollableRefs } = getBinder(id);
  return nestedScrollableRefs[iFocused] ? nestedScrollableRefs[iFocused].iFocused : 0;
}

export function getNestedItemOffsetLeft(id, iFocused, nestedIFocused) {
  const { nestedScrollableItems } = getBinder(id);
  return nestedScrollableItems[iFocused] && nestedScrollableItems[iFocused][nestedIFocused] ? nestedScrollableItems[iFocused][nestedIFocused].offsetLeft : undefined;
}

export function getNestedItemOffsetWidth(id, iFocused, nestedIFocused) {
  const { nestedScrollableItems } = getBinder(id);
  return nestedScrollableItems[iFocused] && nestedScrollableItems[iFocused][nestedIFocused] ? nestedScrollableItems[iFocused][nestedIFocused].offsetWidth : undefined;
}
...
export function itemFocusedHandler(id, iFocused, nestedIFocused, callback) {
  ...
  if (direction === CAROUSEL_DIRECTIONS.verticalBidirectional) {
    const focusedItem = scrollableItems[iFocused].ref.querySelector(verticalChildItemWrapper);
    ...
    const prevNestedFocusedItem = nestedScrollableItems[prevIFocused][prevNestedIFocused || 0].ref.querySelector(horizontalChildItemWrapper);
    if (prevNestedFocusedItem) prevNestedFocusedItem.classList.remove(nestedFocusedClassName);

    const nestedFocusedItem = nestedScrollableItems[iFocused][nestedIFocused || 0].ref.querySelector(horizontalChildItemWrapper);
    nestedFocusedItem.classList.add(nestedFocusedClassName);

    nestedScrollableRefs[iFocused].iFocused = nestedIFocused;

    _updateBinder(id, { iFocused, selectedId: focusedItem.id, nestedIFocused, nestedSelectedId: nestedFocusedItem.id, nestedScrollableRefs });
  }
  else {
